...
export default function Navbar({
  ...
  return (
    <header className="client-navbar px-4 sm:px-6 py-0">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative" ref={langRef}>
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold transition-all cursor-pointer"
              title="Change Language"
            >
              <img
                src={lang === 'km' ? 'https://flagcdn.com/w40/kh.png' : 'https://flagcdn.com/w40/us.png'}
                alt=""
                className="w-5 h-3.5 object-cover rounded-sm shadow-xs shrink-0"
              />
          ...
          <button onClick={onOpenNotifs}
            className="relative p-2.5 rounded-xl hover:bg-slate-100 text-slate-500 transition-all cursor-pointer"
            title="Notifications">
          ...
          {/* Avatar + name */}
          <button onClick={onOpenProfile} className="flex items-center gap-2 px-2 py-1.5 rounded-xl hover:bg-slate-100 transition-all cursor-pointer" title="Edit Profile">
            <img src={currentUser.ProfileImage} alt=""
              className="w-7 h-7 rounded-full object-cover border border-slate-200" />
            <span className="hidden sm:block text-xs font-semibold text-slate-700">{currentUser.Name.split(' ')[0]}</span>
          </button>
